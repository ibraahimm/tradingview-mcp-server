"""Sequential-lifecycle layer — evaluates TASI-W1/TASI-W2/TASI-W3 as ONE pipeline of successive filters, not three
independent screens.

The design intent (recorded): collapse -> TASI-W1 (initial recovery ENTRY/timing) -> TASI-W2 (confirmation the
recovery is becoming healthy) -> TASI-W3 (the name has exited the crisis into NORMAL trend). So we must
...
Two complementary views, both produced here:
  ...
  the earlier stage(s) having fired in the PAST (within the link window K). Populations:
      TASI-W1        : every TASI-W1 trigger.
      TASI-W1->TASI-W2    : a TASI-W2 trigger that had a TASI-W1 trigger in [t-K, t] for the same name (enter at the TASI-W2 bar).
      TASI-W1->TASI-W2->TASI-W3: a TASI-W3 trigger that had a *linked* TASI-W2 in [t-K, t] (which itself followed a TASI-W1).
  Answers "is entering at the more-confirmed, later stage better timing?" — never uses the future.

  DIAGNOSTIC (uses the future BY CONSTRUCTION — labelled, not tradeable) — take the TASI-W1 entries and split
  them by whether the name LATER (within K) progressed to TASI-W2 / TASI-W3. Answers "does the TASI-W2/TASI-W3 transition
  identify the TASI-W1 entries that worked, i.e. filter failed recoveries from successful ones?"
...
def link_stages(p1: dict, p2: dict, p3: dict, K: int) -> dict:
    ...
    p1/p2/p3: {sec_id -> sorted list of td_idx where TASI-W1/TASI-W2/TASI-W3 passes}. K: max trading-day gap linking a
    stage to the prior stage. Returns sets of (sec_id, td_idx):
      w1        : all TASI-W1 triggers
      w2        : TASI-W2 triggers with a TASI-W1 in [b-K, b]            (linked, enter-at-TASI-W2)
      w3        : TASI-W3 triggers with a linked TASI-W2 in [c-K, c]     (chain TASI-W1->TASI-W2->TASI-W3, enter-at-TASI-W3)
      w1_conf2  : TASI-W1 entries that LATER (within K) reach TASI-W2     (diagnostic; uses future)
      w1_conf3  : TASI-W1 entries whose lifecycle LATER reaches TASI-W3   (diagnostic; uses future)
    ...
    for name in names:
        ...
        for a in i1:
            w1.add((name, a))
        # tradeable linked TASI-W2: a TASI-W2 bar b with some TASI-W1 a in [b-K, b]
        linked2 = []
        for b in i2:
            if any(a <= b <= a + K for a in i1):
                linked2.append(b)
                w2.add((name, b))
        # tradeable linked TASI-W3: a TASI-W3 bar c with some linked TASI-W2 b in [c-K, c]
        for c in i3:
            if any(b <= c <= b + K for b in linked2):
                w3.add((name, c))
        # diagnostic: a TASI-W1 entry a "confirms TASI-W2" if some TASI-W2 b in (a, a+K]; "confirms TASI-W3" if the chain reaches TASI-W3
        for a in i1:
...
def mark_stages(labeled: pl.DataFrame, dW1, dW2, dW3, K: int) -> pl.DataFrame:
    """Tag `labeled` with the tradeable stage columns (stg_w1/stg_w2/stg_w3) and the diagnostic TASI-W1-entry
