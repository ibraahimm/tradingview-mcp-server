...
**kept** — same universe of corrected names, one wave further along. The trend dimension
**inverts**: wave 2 *requires* `Perf.1Y > 0` (a confirmed uptrend), where wave 1 preferred
`Perf.3Y ≤ 0`.
...
2. **Run the server-side screen** with `mcp__tradingview-screener__screen_stocks`:
   ...
   - `filters`:
     ...
     - `{ field:"Perf.1M", operator:"greater", value:<p1m_min> }`  (default 0)
